package com.bjw.contact.exec.list;
...
public class ListV3 {
	public static void main(String[] args) {
		 ...
		 * List<Generictype> : List 객체에 add 할수 있는
		 * 				data type을 강제 지정하기
		 * 
		 * Generic type 이외의 데이터를 add() 하면
		 * 코드에 오류가 표현된다
		 * 개발자로 하여금 잘못된 데이터를 add() 하여
		 * 이후에 발생할수 있는 문제를 사전에 방지할수 있도록 알려준다
		 * 
...
//		boolean if = ture;
		...
		/**
		 * 정수형 Genenric type 을 갖는 List 를 하나 만들고 싶다
		 * 그런데 Generic type 에는 primitive keyword 를 사용할 수 없단다
		 * 그럼 어떻게 정수형 Generic type 을 갖는 List 를 만들어야 할까
		 * 
		 * 이러한 이유로 변수를 선언하는 class 를 제공해 준다
		 * 변수를 선언하는 class 는 
		 * primitive(기본) keyword 와 같거나 유사하며 첫글자가 대문자이다
		 * 기본형 변수에 여러개 추가된 확장기능을 첨가하여 만들어진 클래스 이다
		 * 
