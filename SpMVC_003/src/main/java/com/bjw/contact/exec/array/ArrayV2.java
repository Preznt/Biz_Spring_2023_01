...
public class ArrayV2 {
	...
	public static void main(String[] args) {
		// new 내가 선언한 객체를 초기화하는 것
		int[] nums1 = new int[10];
		// 배열 자체를  println() 으로 console 출력을 하면
		// 배열 요소의 0번째 위치의 실제 기억장소 주소를 알수 있다.
		...
		/**
		 * 배열.length (length 속성) : 배열의 개수, 길이를 알려주는 속성이다
		 */
		System.out.println(nums1.length);
		
		
		// nums1 이라는 배열 전체의 모든 요소에 정수 10을 저장하고 싶다
	
		for(int index = 0; index < 10; index++) {
			nums1[index] = 10;
		}
		
		// 배열의 개수를 일일이 확인하지 않아도 전체 배열에 정수 10을 저장할 수 있다
		for(int index = 0 ; index < nums1.length ; index++) {
			nums1[index] = 10;
		}
		/**
		 * 어떤 코드를 사용할때 일반적인 숫자표현식(숫자 리터럴)을 사용하는 것은
		 * 지양(자제) 해야한다
		 ...
		 * 
		 * 매직넘버링 : 숫자를 직접 사용하는 것
		 * 코딩을 할떄 매직넘버링은 잠재적인 오류를 발생할 확률이 매우 높은 방법이다
		 * 가능하면 사용하지 말라
		 */
	}
}
